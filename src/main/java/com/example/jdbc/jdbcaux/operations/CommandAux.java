...
abstract class CommandAux {
    
    protected void genericChekingAnnotations(Object entity) throws Exception {
                ...
                for (Field f : field) {
                            ...
                            if(  f.isAnnotationPresent(JdbcColumn.class) &&  entry.getKey().equals(f.getAnnotation(JdbcColumn.class).value())  ){
                                ...
                                for (Field fFk : fieldsFk){
                                    if (fFk.isAnnotationPresent(JdbcIdentity.class)){
                                        if ( f.getType().isAssignableFrom(Integer.class) ){
                                            f.set(fkObj,  Integer.valueOf( String.valueOf( entry.getValue()) ));
                                        }else if ( f.getType().isAssignableFrom(Long.class) ){
                                            f.set(fkObj,  Long.valueOf( String.valueOf( entry.getValue()) ));
                                        }
                                    }
                                }
